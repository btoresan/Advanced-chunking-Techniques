...
class BaseEvaluation:
    ...
    def _get_chunks_and_metadata(self, splitter):
        ...
        return documents, metadatas, total_time
    ...
    def _scores_from_dataset_and_retrievals(self, question_metadatas, highlighted_chunks_count):
        ...
        for (index, row), highlighted_chunk_count, metadatas in zip(self.questions_df.iterrows(), highlighted_chunks_count, question_metadatas):
            ...
            for metadata in metadatas[:highlighted_chunk_count]:
                ...
                i += 1
                ChunkRelevancy = 0   #size of the relvant part of the current iteration's chunk
                ChunkSize = sum_of_ranges([(chunk_start, chunk_end)])
                ...
                if chunk_corpus_id == corpus_id:
                    ...
                    for ref_obj in references:
                        ...
                        if intersection is not None:
                            ...
                            ChunkRelevancy += sum_of_ranges([intersection])

                if ChunkSize != 0:    
                    ChunkPrecisions.append(ChunkRelevancy/ChunkSize)
                else:
                    ChunkPrecisions.append(0)
                ChunkDCGset = [(x / np.log2(i+2)) for i, x in enumerate(ChunkPrecisions)]
                ChunkDCG = sum(ChunkDCGset)

            PatKs.append(sum(ChunkPrecisions)/len(ChunkPrecisions))
            Sizes.append(ChunkSize) 
            DCGs.append(ChunkDCG)             
        ...
        return Sizes, DCGs, PatKs,iou_scores, recall_scores, precision_scores
    ...
    def _chunker_to_collection(self, chunker, embedding_function, chroma_db_path:str = None, collection_name:str = None):
        ...
        docs, metas, chunking_time = self._get_chunks_and_metadata(chunker)
        ...
        return collection, chunking_time
    ...
    def run(self, chunker, embedding_function=None, retrieve:int = 5, db_to_save_chunks: str = None):
        ...
        if db_to_save_chunks is not None:
            ...
            try:
                ...
                collection, chunking_time = self._chunker_to_collection(chunker, embedding_function, chroma_db_path=db_to_save_chunks, collection_name=collection_name)

        if collection is None:
            collection, chunking_time = self._chunker_to_collection(chunker, embedding_function)
        ...
        Sizes, DCG_scores, PaK_scores, iou_scores, recall_scores, precision_scores = self._scores_from_dataset_and_retrievals(retrievals['metadatas'], highlighted_chunks_count)
        ...
        dcg_mean = np.mean(DCG_scores)
        dcg_std = np.std(DCG_scores)
